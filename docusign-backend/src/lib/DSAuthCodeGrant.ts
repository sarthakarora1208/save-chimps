...
DSAuthCodeGrant.prototype.oauth_callback2 = async function _oauth_callback2(
  req,
  res,
  next
) {
  ...
  const { id, name, email, accounts, accessToken, refreshToken } = req.user;
  ...
  if (!user) {
    let user = await userRepository.create({
      ...
      email,
      id,
      role: USER_ROLE.ARCGIS_USER_ROLE,
    });
